...
@implementation BKCollectionViewCell

+ (instancetype)dequeueReusableCellWithCollectionView:(UICollectionView *)collectionView indexPath:(NSIndexPath *)indexPath {
    BKCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:self.className forIndexPath:indexPath];
...
+ (CGSize)size {
    if (IPhone_6) {
        return CGSizeMake(kScreenWidth, 42.f);
    } else if (IPhone_6P) {
        return CGSizeMake(kScreenWidth, 45.f);
    }
    return CGSizeMake(kScreenWidth, Layout_Width(40.f));
}
